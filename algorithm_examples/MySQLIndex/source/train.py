...
from algorithm_examples.MySQLIndex.source.model import MySQLModel, MySQLModelPairWise
...
def training_pairwise_pilot_score(lero_model, X1, X2, num_epoch):
    ...
    if tuning_model:
        ...
        feature_generator = FeatureGenerator()
        feature_generator.fit(X1 + X2)
        mysql_model = MySQLModelPairWise(feature_generator)
    ...
    mysql_model.fit(X1, X2, Y1, Y2, tuning_model, num_epoch)
